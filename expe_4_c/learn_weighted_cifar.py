...
so around 6.75 hours on my laptop. Much faster on the server.
"""
...
@click.option(
    "--weights_to_load",
    default="weights/weights.txt",
    help="Name of the file that contains weights for the training db.",
)
...
def main(
    ...
    n_classes = 100 if db_name == "cifar100" else 10
    ...
    model = resnet56(n_classes).to(device)  # Number of classes = 100
    ...
    with open(weights_to_load, "rt") as f:
        for line in f.read().strip().split("\n"):
            index, cweight = line.split(" ")
            index, cweight = int(index), float(cweight)
            index_to_weight[index] = cweight

    weights = np.zeros(cifar_train.data.shape[0])
    for index, cweight in index_to_weight.items():
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        pin_memory=False,
        sampler=WeightedRandomSampler(weights, cifar_train.data.shape[0]),
    )
